...
const DisplayStep = ({
  ...
  return (
    <Tool defaultOpen={["suspend", "success"].includes(step.status)}>
      <ToolHeader
...
const ResearchWorkflowChatbot = () => {
  ...
  // 最新のrunIdを取得(ワークフロー再開や承認/却下のAPI呼び出しで使う)
    ...
    return (
      <div className="flex h-screen flex-col justify-end gap-6 px-12 py-10 bg-[#F5F5F5] dark:bg-background">
        <div className="flex flex-1 flex-col items-center justify-center gap-4">
          ...
          <h1 className="font-serif text-[32px] italic text-foreground">
            何をリサーチしましょうか？
          </h1>
          <p className="text-sm text-[#888888]">
            トピックを入力するか、下のサジェストから選んでください
          </p>
        </div>
        {/* サジェストボタン群 */}
      ...
      <div>
        ...
        {isWaitingForApproval ? (
          <div className="flex gap-3">
            <PromptInputButton onClick={handleApprove} className="flex-1">
